...
> **Note 1:** This project is developed on Windows 10 using UnrealEngine 4.12.5. It should work on Windows as well :ok_hand:.
...
It is recommended that you always create multiple `fbx` files so that the UnrealEngine doesn't crash (if the project is big). In this case, it's not a big import, so I didn't do that.
...
Let try to give a shine to one of the materials which is used in as texture to the outside railings. Do the following:
...
> Note 5: If you are in Windows you can only package the game for Windows only but not for Mac and Linux. It goes the same to Mac and Linux.
